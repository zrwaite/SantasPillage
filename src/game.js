...
export default class Game {
  ...
  start() {
    ...
    if(this.level===0){
      ...
      this.cutscene = new Cutscene(this.players, 0, this.info ,this.display)
      this.objects = [...this.blocks, ...this.doors, ...this.canes, ...this.icicles, this.cutscene.santa, ...this.cutscene.persons, ...this.cutscene.elves, ...this.cutscene.snowmen, ...this.cutscene.gingers, ...this.cutscene.deers]
      this.delObjects = [...this.cookies]
      return;}
    if (this.level===this.startLevel+1){this.startTime=new Date().getTime()}
    ...
    if(this.level===11){this.lives[0]+=3}
  ...
  update(deltaTime) {
    if (this.state===this.states.start){this.display.update(deltaTime)}
      ...
      this.cutscene.persons.forEach((person)=>this.personCutscene(person));
      this.cutscene.elves.forEach((elf)=>this.elfCutscene(elf));
      this.cutscene.snowmen.forEach((snowman)=>this.snowmanCutscene(snowman));
      this.cutscene.deers.forEach((deer)=>this.deerCutscene(deer));
      this.cutscene.gingers.forEach((ginger)=>this.gingerCutscene(ginger))
      this.doors.forEach((door)=>this.doorCutscene(door))
      this.cookies.forEach((cookie)=>this.cookieCutscene(cookie))
      this.icicles.forEach((icicle)=>this.icicleCutscene(icicle))
      this.snowballs.forEach((snowball)=>this.snowballGame(snowball))
      this.santaCutscene(this.cutscene.santa);
      [this.bg, ...this.objects, ...this.delObjects, this.display].forEach((object) =>object.update(deltaTime));
      ...
      [...this.objects, ...this.delObjects].forEach((object)=>(object.pos.x=object.pos.x===null? null:object.realPos-this.pos));
    }
  ...
  draw(ctx) {
    ...
    if (this.state===this.states.cutscene){
      [this.bg, ...this.objects, ...this.delObjects, this.display].forEach((object) => object.draw(ctx))
  ...
  personCutscene(person){
    ...
    if (person.realPos >= 450 && person.realPos<=450+this.levelLen && person.lives>0){
      this.cutscene.persons.forEach((object)=>pPoss.push(object.lives>0? object.realPos:0))
  ...
  santaCutscene(santa){
    santa.detects.top=false
    this.blocks.forEach((block)=>this.personBlock(santa, block))
  }
  deerCutscene(deer){
  ...
  doorCutscene(door){
    ...
    for (let i=0; i<this.numPlayers; i++){
      if(this.detector(this.cutscene.persons[i], door)!==false&&door.state==='open'){
        this.level ++
        this.start()
        break;
      }
  ...
  cookieCutscene(cookie){
    this.cutscene.persons.forEach((person)=>this.personCookie(person, cookie))
  }
  gingerCutscene(ginger){
    ...
    if(ginger.jumpCount === 1){
      ...
      if (this.numPlayers === 1){
        if(ginger.realPos < this.cutscene.persons[0].realPos){ginger.dir = ginger.dirs.right}
        else {ginger.dir = ginger.dirs.left}
      } else if (this.numPlayers === 2){
        this.cutscene.persons.forEach((object)=>pPoss.push(object.lives>0? object.realPos:-10000))
  ...
  icicleCutscene(icicle){
    if(icicle.mstate==="i1"){this.cutscene.persons.forEach((person)=>this.verticalDetector(person, icicle))}
    if(icicle.falling){this.blocks.forEach((block)=>this.icicleBlock(icicle, block))}
  }
  personGame(person){
    person.detects.top=person.detects.left=person.detects.right=person.detects.bottom=false
    this.blocks.forEach((block)=>this.personBlock(person, block));
    if(person.lives<=0){return}
    ...
    if (person.realPos >= 450 && person.realPos<=450+this.levelLen && person.lives>0){
      for(let i=0; i<this.persons.length; i++){
        if(this.persons[i].lives>0){pPoss.push(this.persons[i].realPos)}
      }
      if (Math.max(...pPoss) > this.levelLen + 450){this.pos = this.levelLen}
      else{this.pos = Math.max(...pPoss) - 450}
    } else {//Not in da zone
      ...
      //else if (this.lives[0]+this.lives[1]-person.lives<=0&&person.lives>0&&this.numPlayers>1){this.pos = 0;console.log("problem")}
    }
  }
  elfGame(elf){
    if(elf.realPos + elf.width >= this.levelLen+800){elf.dir = elf.dirs.left}
  ...
  snowmanGame(snowman){
    ...
    if(snowman.throw){
      ...
      let pPoss = [];
      if (this.numPlayers === 1){
        if(snowman.realPos < this.persons[0].realPos){dir = 1}
      } else if (this.numPlayers === 2){
        this.persons.forEach((object)=>pPoss.push(object.lives>0? object.realPos:-10000))
        if (Math.abs(snowman.realPos-pPoss[0])>Math.abs(snowman.realPos-pPoss[1])){pPoss = pPoss[1]}
        else{pPoss = pPoss[0]}
        if(snowman.realPos < pPoss){dir = 1}
      }
      this.snowballs.push(new Snowball(dir, this.info, this.snowballi, {x:snowman.realPos+snowman.width/2, y: snowman.pos.y+snowman.height/2}))
      this.delObjects.push(this.snowballs[this.snowballs.length-1])
      this.snowballi++
  ...
  icicleGame(icicle){
    if(icicle.mstate==="i1"){this.persons.forEach((person)=>this.verticalDetector(person, icicle))}
    if(icicle.falling){this.blocks.forEach((block)=>this.icicleBlock(icicle, block))}
  }
  deerGame(deer){
    if(deer.realPos + deer.width >= this.levelLen+800){deer.dir = deer.dirs.left}
  ...
  doorGame(door){
    ...
    for (let i=0; i<this.numPlayers; i++){
      if(this.detector(this.persons[i], door)!==false&&door.state==='open'){
        if(this.numPlayers>1){this.points[i]++}
        this.level ++
        this.start()
        break;
      }
  ...
  icicleBlock(icicle, block){
    let spot = this.detector(icicle, block, -1, 10)
    if(spot==="top"){
      icicle.detect=true
